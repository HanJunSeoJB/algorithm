...
Menu = Enum('Menu', ['머리노드에삽입', '꼬리에노드삽입', '머리노드삭제',
                     '꼬리노드삭제', '주목노드출력', '주목노드이동','주목노드삭제',
                     '모든노드삭제', '검색', '모든노드출력', '스캔', '종료', '멤버십판단'])
...
while True:
    ...
    if menu == Menu.머리노드에삽입:
        ...
        print('그 값의 데이터는 포함되어' + (' 있습니다. ' if int(input('판단 할 값을 입력: ')) in lst else ' 있지 않습니다. '))
